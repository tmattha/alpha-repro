...
module load GCC CUDA ParaStationMPI MPI-settings/CUDA UCX-settings/RC-CUDA Nsight-Systems
...
srun --cpu-bind=threads --distribution=block:cyclic:block --cpus-per-task=${SLURM_CPUS_PER_TASK} \
    nsys profile -o $PROJECT/matthaei1/nsys/twonode_gmres-%q{SLURM_JOB_ID}-%q{SLURM_PROCID} \
    -t "cuda,nvtx,openmp,mpi" \
